...
//varying vec4 myLambert;
varying vec4 myColor;

// These are passed in from the CPU program, camera_control_*.cpp

//he 1per ver/1per pixel phong vs gouro || phong per frag || gourad per vertex
/*
 ...
 vec4 gouraud = myspecular * lightcolor * pow (max(nDotH, 0.0), myshininess);
 
 vec4 retColor =  gouraud + myLambert;
 return retColor;
 }
 */
void main (void){
    ...
    /*
     ...
     //vec4 lambert1 = diffuse * light0_color * max (nDotL, 0.0);
     //vec4 lambert2 = diffuse * light1_color * max (nDotL, 0.0);
     gl_FragColor = ambient + color0 + color1; //+ lambert1 + lambert2;
     */
